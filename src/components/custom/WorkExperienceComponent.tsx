    ...
    <div className="space-y-4">
      {experiences.map((experience, index) => (
        <div key={index} className="border-b border-gray-300 pb-6">
          <h3 className="text-lg font-semibold text-primary">{experience.company}</h3>
            ...
            {experience.roles.map((role, roleIndex) => (
              <>
                <div key={roleIndex} className="flex justify-between">
                  ...
                  <div className="text-xs  text-right text-gray-500 min-w-[6rem] md:min-w-64 2xl:min-w-fit">
                    <p>{role.address}</p>
                    <p>{role.duration}</p>
                  </div>
                </div>
